...
{
    ScreenUtility::SetCursor(0, Y_BASE - yPos); // 공룡의 높이에 따라 출력 위치 조정

    // "D"자를 사용해서 공룡의 모양 출력
    cout << "    D\n";
    cout << "    DD\n";
    cout << "DDDDD\n";
    // 매 프레임마다 공룡의 발 모양을 변화시킴
    if (footToggle)
        cout << "D\n";
    else
        cout << "    D\n";
...
{
    jumpSpeed = 1.0f + baseSpeed * 0.3f; // 게임 속도의 15%만큼 점프 속도 설정
    fallSpeed = 1.2f + baseSpeed * 0.35f; // 게임 속도의 10%만큼 낙하 속도 설정
}
