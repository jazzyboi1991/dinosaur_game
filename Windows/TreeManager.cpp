...
{
    srand((unsigned)time(nullptr)); // 난수 초기화
    gapThreshold = static_cast<float>(rand() % 20 + 40); // 초기 간격 설정
}
...
void TreeManager::update(float speed) // 나무 이동, 생성, 제거 처리 (매 프레임마다 실행됨)
{
    ...
    if (gapCounter >= gapThreshold && trees.size() < 4) // 일정 간격이 지나고 나무가 너무 많지 않으면
    {
        gapThreshold = static_cast<float>(rand() % 10 + 10); // 새 간격 랜덤 재설정
        float chance = static_cast<float>(rand() % 21 + 80) / 100.0f; // 출현 확률 랜덤화
        
        // 계산된 확률보다 작으면 나무 생성
        if ((rand() % 100) < static_cast<int>(chance * 100))
        {
            trees.emplace_back(Tree()); // 새로운 나무 생성
        }
